...
out_arr(1) = outdata;
% Note: From here onwards, is_error is a debugging tool. It does not actually affect the
% program's operation.
...
while t < SimDur
    ...
    if output
        % I don't want the simulation stopping just because the plot function broke for
        % some reason. If debug==0, then a failure in the plot function will not stop the
        % simulation.
        if debug
