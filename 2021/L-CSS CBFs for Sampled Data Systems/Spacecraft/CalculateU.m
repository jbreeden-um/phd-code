...
% bw = [(constants.w_max - x(4:6))/dt; (constants.w_max + x(4:6))/dt];
    % None of the simulations violate the assumption encoded by the above constraint anyways,
    % but applying this makes OSQP fail more often during the simulations that break.
    % Applying the above constraint will not affect the simulations that do not break.
